...
writeCDF(temp_masked, here("data", "temp.nc"))
...
clc_nl$code_label <- code_labels[as.character(clc_nl$Code_18)]

# Plot for first impression
top_labels <- names(sort(tapply(clc_nl$Area_Ha, clc_nl$code_label, sum), decreasing = TRUE))[1:7]
clc_nl$filtered_label <- ifelse(clc_nl$code_label %in% top_labels, clc_nl$code_label, "Others")

palette_clc <- hsv(h = seq(0, 1-1/length(unique(clc_nl$filtered_label)), length.out = length(unique(clc_nl$filtered_label))),
                   s = 0.6, v = 0.9)

names(palette_clc) <- unique(clc_nl$filtered_label)
...
if (status_code(token_response) != 200) {
    stop("Request for token failed", " ",status_code(token_response), "\nPlease check your token.json")
}
...
repeat {
  ...
  # Wait
  message("Request not yet finished. Retry in 60 seconds..")
  Sys.sleep(60)
}
...
|                                    |                      | nld_2_sf: sf-Object             |                                            |             |
+------------------------------------+----------------------+---------------------------------+--------------------------------------------+-------------+
| CHELSA (temperature data)          | Yes                  | Raster                          | 0.008333333                                | WGS 84      |
|                                    |                      |                                 |                                            |             |
| --\> Average temperature in 2019   |                      | temp: SpatRaster (terra) | \~ 930 m (North-South) x 570 m (East-West) |             |
|                                    |                      |                                 |                                            |             |
|                                    |                      | temp_df: DataFrame       |                                            |             |
+------------------------------------+----------------------+---------------------------------+--------------------------------------------+-------------+
| HDX (population density)           | Yes                  | Raster                          | 0.008333333                                | WGS84       |
|                                    |                      |                                 |                                            |             |
| 2019                               |                      | pop2:                           | \~ 930 m (North-South) x 570 m (East-West) |             |
+------------------------------------+----------------------+---------------------------------+--------------------------------------------+-------------+
| Corine (Land Use)                  |                      | Vector\                         |                                            |             |
|                                    |                      | :                               |                                            |             |
+------------------------------------+----------------------+---------------------------------+--------------------------------------------+-------------+
...
# Temperature
temp <- raster(here("data", "temp.nc"))
temp_df <- as.data.frame(temp_masked$tas, xy = TRUE, na.rm = TRUE)

# Land Use
clc_nl <- st_read(here("data", "clc_nl.gpkg"))
